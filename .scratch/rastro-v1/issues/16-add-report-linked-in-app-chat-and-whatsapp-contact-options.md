...
- [ ] Report/listing creation lets the creator choose in-app chat, WhatsApp, or both.
- [ ] WhatsApp contact opens WhatsApp using only the phone number the creator chose to expose.
- [ ] Phone verification is not required.
- [ ] In-app chat is one-to-one and tied to a report or listing.
- [ ] Chat does not support public comments, group chat, unrelated DMs, image attachments, or file attachments.
- [ ] New-message alerts use push notifications.
- [ ] Open chat screens refresh on focus/send/polling without always-on sockets.
